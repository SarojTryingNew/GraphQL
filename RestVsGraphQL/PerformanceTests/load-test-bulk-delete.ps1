...
Write-Host "   Bulk DELETE Operations - REST Direct vs REST+GraphQL" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
# Test 1: REST Direct Bulk Delete
Write-Host "`n=== Test 1: REST Direct - Bulk Delete Orders ===" -ForegroundColor White

Invoke-BulkTest -Name "REST Direct - Bulk Delete ($OrdersPerBulk orders per request)" -Count $Iterations -Request {
...
# Test 2: REST with GraphQL Backend Bulk Delete
Write-Host "`n=== Test 2: REST with GraphQL Backend - Bulk Delete Orders ===" -ForegroundColor White

Invoke-BulkTest -Name "REST with GraphQL - Bulk Delete ($OrdersPerBulk orders per request)" -Count $Iterations -Request {
    param($url, $ordersCount)

    # Get oldest orders to delete
    $allOrders = Invoke-RestMethod -Uri "$url/api/graphql-backend/orders" -Method Get -ErrorAction Stop
    $oldOrders = $allOrders | Select-Object -Last $ordersCount
    ...
    $bulkDeleteData = @{ orderIds = $orderIds } | ConvertTo-Json -Depth 10
    Invoke-RestMethod -Uri "$url/api/graphql-backend/orders/bulk" -Method Delete -Body $bulkDeleteData -ContentType "application/json" -ErrorAction Stop
}
...
try {
    ...
    Write-Host "`nREST Direct API (DataStore) - BULK DELETE:" -ForegroundColor Red
    ...
    Write-Host "`nREST with GraphQL Backend - BULK DELETE:" -ForegroundColor Magenta
