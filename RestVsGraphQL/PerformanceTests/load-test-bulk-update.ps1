...
Write-Host "   Bulk UPDATE Operations - REST Direct vs REST+GraphQL" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
# Test 1: REST Direct Bulk Update
Write-Host "`n=== Test 1: REST Direct - Bulk Update Orders ===" -ForegroundColor White

Invoke-BulkTest -Name "REST Direct - Bulk Update ($OrdersPerBulk orders per request)" -Count $Iterations -Request {
...
# Test 2: REST with GraphQL Backend Bulk Update
Write-Host "`n=== Test 2: REST with GraphQL Backend - Bulk Update Orders ===" -ForegroundColor White

Invoke-BulkTest -Name "REST with GraphQL - Bulk Update ($OrdersPerBulk orders per request)" -Count $Iterations -Request {
    param($url, $ordersCount)
    ...
    $bulkUpdateData = @{ orders = $updates } | ConvertTo-Json -Depth 10
    Invoke-RestMethod -Uri "$url/api/graphql-backend/orders/bulk" -Method Put -Body $bulkUpdateData -ContentType "application/json" -ErrorAction Stop
}
...
try {
    ...
    Write-Host "`nREST Direct API (DataStore) - BULK UPDATE:" -ForegroundColor Red
    ...
    Write-Host "`nREST with GraphQL Backend - BULK UPDATE:" -ForegroundColor Magenta
