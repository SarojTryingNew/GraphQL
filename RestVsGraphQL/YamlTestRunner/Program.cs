...
public class Program
{
    public static async Task Main(string[] args)
    {
        ...
        // Get the solution root directory (4 levels up from bin/Debug/net10.0)
        var solutionRoot = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", ".."));
        var testSuitesPath = Path.Combine(solutionRoot, "RestVsGraphQL", "TestSuites");

        switch (choice)
        {
            case "1":
                await RunTestSuite(runner, Path.Combine(testSuitesPath, "rest-tests.yaml"));
                break;
            case "2":
                await RunTestSuite(runner, Path.Combine(testSuitesPath, "graphql-tests.yaml"));
                break;
            case "3":
                await RunTestSuite(runner, Path.Combine(testSuitesPath, "comparison-tests.yaml"));
                break;
            case "4":
                await RunTestSuite(runner, Path.Combine(testSuitesPath, "rest-tests.yaml"));
                await RunTestSuite(runner, Path.Combine(testSuitesPath, "graphql-tests.yaml"));
